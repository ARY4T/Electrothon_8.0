...
const TETRIS_PIECES = [
  ...
  { shape: [[1, 1], [1, 1]], color: 'bg-white' },
  // T-piece
  { shape: [[0, 1, 0], [1, 1, 1]], color: 'bg-white' },
  // L-piece
  { shape: [[1, 0], [1, 0], [1, 1]], color: 'bg-white' },
  // S-piece
  { shape: [[0, 1, 1], [1, 1, 0]], color: 'bg-white' },
  // Z-piece
  { shape: [[1, 1, 0], [0, 1, 1]], color: 'bg-white' },
  // J-piece
  { shape: [[0, 1], [0, 1], [1, 1]], color: 'bg-white' },
]
      ...
      if (linesToClear.length > 0) {
        ...
        const newGrid = prevGrid.map((row, rowIndex) => {
          if (linesToClear.includes(rowIndex)) {
            return row.map(
              cell => ({ ...cell, color: 'bg-black dark:bg-white animate-pulse opacity-50' })
            );
          }
          return row
        })
      ...
      <div key={rowIndex} className="flex">
        {row.map((cell, colIndex) => (
          <div
            ...
            className={`${config.cellSize} border-[0.5px] border-zinc-800/50 transition-all duration-100 ${
              cell.filled 
                ? `bg-white scale-100 shadow-[0_0_8px_rgba(255,255,255,0.3)]` 
                : 'bg-zinc-950/20 scale-100'
